...
- **Partie V — Dashboards et Monitoring** : visualiser tout avec 1 dashboard unifié (4 onglets, 9 sources, 20 KPI)
- **Partie VI — Automatisation et Crons** : faire tourner 16 tâches planifiées sans lever le petit doigt
...
| | Chatbot | Agent |
|:--|:-------:|:-----:|
| **Vous dites** | "Quel temps fait-il à Paris ?" | "Prépare un email pour Paul avec le compte-rendu de la réunion d'hier." |
| **Réaction** | Répond : "Il fait 22°C et ciel dégagé." | Cherche le document → Rédige le résumé → Ouvre Gmail → Envoie l'email |
| **Verbe** | 📢 **PARLE** — mais n'agit pas | 🦁 **PARLE ET AGIT** |
...
|:--------|:------------------------------------------|
| À 07:00 | Collecte + analyse veille IA (15 sources RSS) |
| À 08:00 | Alerte budget si solde < 10€ |
| Collecte */15 | collect-v2.py unifie 9 sources (sessions, budget, crons, infra, n8n, github, bavi, services, vaults) |
| Déploiement H:10 | Deploy Unified Dashboard sur GitHub Pages |
| Toutes les 15 min | Auto-Heal Agent : vérifie santé des 16 crons, services, disque |
| Toutes les 30 min | Synchronisation des skills entre profils |
| Toutes les 60 min | Vérification n8n healthcheck |
| À 18:00 | Synchronisation Drive ↔ GitHub |
...
LEO a **16 crons actifs** tous en statut OK (0 erreur, 0 en pause). 3 crons sur le profil `default`, 13 sur `leo-copilot`. La majorité tournent en `no_agent` (0$ de consommation LLM pour les tâches répétitives).
...
| `emile` | @Bureau_ia_emilie_bot | DeepSeek Flash + Gemini | Pédagogie mémoire |
...
```mermaid
flowchart TB
    TG[📱 Telegram] --> GW[Gateway Hermes]
    GW --> D["Profil default<br/>DeepSeek Flash"]
    GW --> LC["Profil leo-copilot<br/>DeepSeek V4 Pro"]
    GW --> BL["Profil bavi-leo<br/>DeepSeek Flash"]
    GW --> EM["Profil emile<br/>DeepSeek Flash + Gemini"]

    D -->|fallback| GM[Gemini 3.5 Flash]
    LC -->|fallback| GM
    BL -->|fallback| GM
    EM -->|contexte >128K| GM

    D -->|batch gratuit| OL[Ollama<br/>qwen2.5:7b<br/>local CPU]
    LC -->|crons no_agent| S2[Scripts shell<br/>0$ LLM]
```
...
| 🎓 @Bureau_ia_emilie_bot | `emile` | DeepSeek Flash + Gemini | Pédagogie mémoire | < 2s | Gratuit/Payant |
...
flowchart TD
    subgraph "🐳 Docker Container"
        direction TB
        S6[s6 supervision<br/>auto-restart] --> GW[hermes-gateway]
        GW --> P1[default]
        GW --> P2[leo-copilot]
        GW --> P3[bavi-leo]
        GW --> P4[emile]
        S6 --> LOG[s6-log<br/>rotation auto]
        S6 --> CR[cron scheduler<br/>Hermes natif]
    end

    subgraph "🌐 Services"
        N8N[n8n<br/>localhost:5678]
        OL[Ollama<br/>qwen2.5:7b<br/>port 11434]
        NG[Nginx<br/>Cloudflare Tunnel]
    end
...
### Le 1 dashboard (4 onglets)

Un seul dashboard HTML statique hébergé sur **GitHub Pages** — zéro backend, zéro base de données :
...
### Les 16 crons (tâches planifiées)

> 16/16 OK, 0 erreur, 0 en pause. 3 crons sur le profil `default`, 13 sur `leo-copilot`. La majorité en `no_agent` = **0$ par mois** de consommation LLM pour les tâches répétitives.

| Vague | Horaires | Crons |
|:------|:---------|:------|
| **15 min** | */15 | Unified Collector v2, Auto-Heal Agent, LEO Health Check |
| **30 min** | */30 | Email Classifier, sync-skills-to-copilot |
| **Horaire** | H:10 | Deploy Unified Dashboard, Déploiement auto tofdan.be, drive-sync |
| **2-6h** | */2, */4, */6 | docs-update, doc-watch-auto, Archive Watch |
| **Quotidien** | 02:00, 07:00 | backup complet, veille IA |
| **Bi-quotidien** | 08:00, 20:00 | Budget Alert |
| **Spécial** | 23:00 | vault-daily-journal (default + leo-copilot) |
...
**Erreur :** Un proxy compliqué et instable entre Hermes et Gemini (via LLM gateway intermédiaire).
...
| Métrique | Valeur |
|:---------|:-------|
| Crons actifs | **16** (16/16 OK, 0 erreur, 0 pause) |
| Skills installés | **126** |
| Dashboards | **1** (central 4 onglets ✅, 9 sources unifiées) |
| Workflows n8n | **3** (3 actifs) |
| Vaults Obsidian | **4** (leo, default, emile, bavi) |
| Profils bots | **4** (default, leo-copilot, bavi-leo, emile) |
| Wikis | **5** (98 pages total) |
| Repos GitHub | **20** |
| Consommation DeepSeek totale | **~$19.99 estimé** (tous profils) |
| Machine hôte | **1** (serveur LEO uniquement) |

## 📝 À retenir

- LEO = 1 serveur principal + 4 bots Telegram + 1 dashboard central (4 onglets) + 16 crons + 126 skills
...
```mermaid
flowchart LR
    subgraph "Un seul bot (déconseillé)"
        A1[🦁 LEO<br/>Tout mélanger]
    end
    subgraph "4 bots spécialisés ✅"
        B1[🦁 LEO<br/>Hub central]
        B2[🟪 Copilote<br/>Infrastructure]
        B3[🧭 Sylvia<br/>Voyages]
        B4[🎓 Émile<br/>Pédagogie]
    end
```
...
- **Émile** (emile) : l'assistant pédagogique — mémoire universitaire, relecture

### 2. Modèles adaptés à chaque usage

| Bot | Modèle principal | Coût | Usage typique |
|:----|:-----------------|:----:|:--------------|
| LEO | DeepSeek V4 Flash | ~$0.02/j | Quotidien, polyvalent |
| Léo Copilote | DeepSeek V4 Pro | ~$0.15/j | Analyses complexes, infra |
| Sylvia | DeepSeek V4 Flash | ~$0.01/j | Roadbooks, voyages |
| Émile | DeepSeek Flash + Gemini | ~$0.01/j | Pédagogie, longs contextes |
...
| Profil | Token | Utilisateur autorisé |
|:-------|:------|:---------------------|
| `default` | `881242...` | Christophe |
| `leo-copilot` | `899720...` | Christophe |
| `bavi-leo` | `885780...` | Christophe + Sylvie |
| `emile` | `890688...` | Émilie |

## Flow de routage entre les 4 bots

```mermaid
flowchart TD
    USER[👤 Christophe<br/>Telegram] -->|message| ROUTER{🤖 LEO Hub<br/>default}
    
    ROUTER -->|analyse/infra/cron| COPILOT[🟪 Léo Copilote<br/>leo-copilot<br/>DeepSeek V4 Pro]
    ROUTER -->|voyage/roadbook| SYLVIA[🧭 Sylvia<br/>bavi-leo<br/>DeepSeek Flash]
    ROUTER -->|pédagogie/mémoire| EMILE[🎓 Émile<br/>emile<br/>DeepSeek Flash + Gemini]
    ROUTER -->|quotidien/général| LEO[🦁 LEO<br/>default<br/>DeepSeek Flash]
    
    COPILOT -->|déploiement| DASH[📊 Dashboard<br/>GH Pages]
    COPILOT -->|supervise| CRONS[⏱️ 16 crons<br/>16/16 OK]
    COPILOT -->|gère| N8N[🔄 n8n<br/>3 workflows]
    COPILOT -->|surveille| VAULTS[📓 4 vaults<br/>Obsidian]
    
    SYLVIA -->|produit| ROAD[🗺️ Roadbooks<br/>Voyages Wiki]
    EMILE -->|assiste| MEMOIRE[📖 Mémoire<br/>universitaire]
    
    LEO -->|orchestre| BAVI[🏛️ 10 bureaux<br/>BAVI]
    LEO -->|classe| EMAIL[📧 Emails<br/>9 catégories]
    LEO -->|batch gratuit| OLLAMA[🦙 Ollama<br/>qwen2.5:7b]
```

## Architecture des profils

Maintenant, concrètement, comment créer ces profils et configurer les bots ? Voici les étapes.
...
| `leo-copilot` | Léo Copilote 🟪 | DeepSeek V4 Pro | Infrastructure — 16 crons, système, budget, 3 n8n |
| `bavi-leo` | Sylvia 🧭 | DeepSeek V4 Flash | Roadbooks camping-car, voyages |
| `emile` | Émile 🎓 | DeepSeek V4 Flash + Gemini | Assistant pédagogique mémoire universitaire |
...
├── 🔧 16 crons automatisés (16/16 OK)
├── 📊 1 dashboard unifié (9 sources, 4 onglets, 20 KPI)
├── 🔄 3 workflows n8n (3 actifs)
├── 🌐 Nginx + Cloudflare Tunnel
├── 🔒 UFW + SSL + DNS
├── 💰 Suivi du budget DeepSeek (réel: ~$19.99)
...
Les crons sont le cœur de l'automatisation. **16 tâches planifiées tournent 24/7, 16/16 OK, 0 erreur, 0 en pause.**

### Tous les 16 crons (détail)

| Cron | Profil | Horaire | Statut | Runs 7j |
|:-----|:------|:--------|:------:|:-------:|
| 🩺 LEO Health Check bots | default | `*/15 * * * *` | ✅ OK | 289 |
| 📓 vault-daily-journal | default | `0 23 * * *` | ✅ OK | 1 |
| 📓 vault-default-daily-journal | default | `0 23 * * *` | ✅ OK | 1 |
| 💾 LEO Full Backup quotidien | leo-copilot | `0 2 * * *` | ✅ OK | 5 |
| 🔍 Veille IA quotidienne | leo-copilot | `0 7 * * *` | ✅ OK | 5 |
| 🔄 Déploiement auto tofdan.be | leo-copilot | `0 * * * *` | ✅ OK | 100 |
| 📧 Email Classifier (Ollama) | leo-copilot | `*/30 * * * *` | ✅ OK | 199 |
| 📝 docs-update | leo-copilot | `0 */4 * * *` | ✅ OK | 24 |
| 🔄 drive-sync | leo-copilot | `0 * * * *` | ✅ OK | 98 |
| 📖 doc-watch-auto | leo-copilot | `0 */6 * * *` | ✅ OK | 18 |
| 🩺 Auto-Heal Agent | leo-copilot | `*/15 * * * *` | ✅ OK | 390 |
| 🔄 sync-skills-to-copilot | leo-copilot | `*/30 * * * *` | ✅ OK | 200 |
| 📦 Archive Watch — leo-tracker | leo-copilot | `0 */6 * * *` | ✅ OK | 19 |
| 📊 Unified Collector v2 | leo-copilot | `*/15 * * * *` | ✅ OK | 350 |
| 🚀 Deploy Unified Dashboard | leo-copilot | `10 * * * *` | ✅ OK | 143 |
| 💰 Budget Alert | leo-copilot | `0 8,20 * * *` | ✅ OK | 6 |
...
Sur 16 crons, **13 sont en no_agent** — le coût total des crons est d'environ **quelques centimes par jour**, limité à la veille IA (DeepSeek Flash).

## Cycle de collecte → Déploiement → Dashboard (BPMN)

```mermaid
flowchart LR
    subgraph "🔄 Cycle complet (15 min + H:10)"
        A1[collect-v2.py<br/>*/15 * * * *] -->|9 sources unifiées| A2[leo-unified.json<br/>/opt/data/metrics/]
        A2 -->|H:10<br/>10 * * * *| A3[Deploy Unified Dashboard<br/>deploy-dashboard.py]
        A3 -->|git add + commit + push| A4[GitHub Pages<br/>CDN rebuild]
        A4 -->|serve| A5[🌍 LEO Dashboard<br/>christophedanhier-hash.github.io/leo-dashboard/]
    end

    subgraph "📡 9 sources collect-v2"
        B1[sessions<br/>state.db × 4]
        B2[budget<br/>API DeepSeek]
        B3[crons<br/>scheduler Hermes]
        B4[infra<br/>CPU/RAM/disque]
        B5[n8n<br/>healthz API]
        B6[github<br/>repos activity]
        B7[bavi<br/>bureaux index]
        B8[services<br/>Spotify...]
        B9[vaults<br/>Obsidian × 4]
    end
...
## Vaults Obsidian — 4 coffres de notes

4 vaults Obsidian, un par profil Hermes, pour la prise de notes structurée avec monitoring intégré :

```mermaid
flowchart TB
    subgraph "📓 4 Vaults Obsidian"
        VL["🦁 vault-leo<br/>profil: leo-copilot<br/>3 notes, 1 daily"] -->|monitoring| COL[collect-v2.py]
        VD["📁 vault-default<br/>profil: default<br/>2 notes, 1 daily"] --> COL
        VE["🎓 vault-emile<br/>profil: emile<br/>2 notes, 1 daily"] --> COL
        VB["🚐 vault-bavi<br/>profil: bavi-leo<br/>2 notes, 1 daily"] --> COL
    end

    COL -->|9 sources| JSON[leo-unified.json]
    JSON --> DASH[LEO Dashboard<br/>carte Vaults]
```

| Vault | Profil | Chemin | Notes | Journaux | Sous-dossiers |
|:------|:-------|:-------|:-----:|:--------:|:--------------|
| 🦁 **vault-leo** | leo-copilot | `/opt/data/vault-leo/` | 3 | 1 | Hermes, Daily, Projets, References |
| 📁 **vault-default** | default | `/opt/data/vault-default/` | 2 | 1 | Hermes, Daily, Projets, References |
| 🎓 **vault-emile** | emile | `/opt/data/vault-emile/` | 2 | 1 | Hermes, Daily, Projets, References |
| 🚐 **vault-bavi** | bavi-leo | `/opt/data/vault-bavi/` | 2 | 1 | Hermes, Daily, Projets, References |

**Structure commune** : `Daily/`, `Projets/`, `References/`, `Hermes/{Config,Crons,Dashboard}`.

**Monitoring** : intégré au collecteur `collect-v2.py` (fonction `collect_vaults()` section #9). Les métriques (9 notes, 4 journaux) apparaissent dans la carte `📓 Vaults Obsidian` de l'onglet Synthèse du dashboard.

**Génération automatique** : les crons `vault-daily-journal` (default) et `vault-default-daily-journal` (leo-copilot) génèrent un journal quotidien dans chaque vault à 23:00.
...
|| **Crons no_agent** | 0 € — 13 sur 16 |
| **Total mensuel estimé** | **~1-3 €/mois** ($19.99 réel estimé depuis le début) |
...
- **n8n v2.26.8 Docker** — **3 workflows actifs**
- **3 credentials** (Google, GitHub, n8n)
- Accès via `localhost:5678`
- **Base SQLite** dans un volume Docker dédié
- **3 workflows** : Drive→Issue GitHub, Gardien du Drive, Save Contacts

```mermaid
flowchart LR
    subgraph "n8n — 3 workflows actifs"
        W1[Drive → Issue GitHub] -->|nouveau fichier| GH[Issue GitHub]
        W2[Gardien du Drive] -->|surveillance| AL[Alertes]
        W3[Save Contacts] -->|sync| GContacts[Google Contacts]
    end
```

> **Consolidation post-crash** : passé de 6 à **3 workflows actifs**. Les workflows redondants ont été supprimés pour plus de stabilité. Tous les 3 sont actifs au 04/07/2026.
...
|:----------|:--------:|:------------:|
| Crons | **16** (16/16 OK) | ~0 €/j (majorité no_agent) |
| Dashboards | 1 central (4 onglets, 9 sources) | 0 € (GitHub Pages) |
| n8n workflows | **3 actifs** | 0 € (self-hosted) |
| Machine hôte | 1 | 0 € (serveur local) |
| Vaults Obsidian | 4 (9 notes, 4 journaux) | 0 € |
| DeepSeek API | Flash + Pro | ~1-3 €/mois ($19.99 réel estimé depuis le début) |
| **Total** | | **~1-3 €/mois** |

> ⚠️ **Budget réel vs affiché** : Au 04/07/2026, le coût réel estimé est d'environ **$19.99** (tous profils confondus depuis le début), contre $0.41 affiché par les anciens dashboards pré-crash. Le nouveau collecteur `collect-v2.py` utilise les données de `estimated_cost_usd` des 4 state.db pour un suivi fiable.
...
LEO a **1 dashboard central** (4 onglets : Synthèse, Analyses, Infra, BAVI — 20 KPI, 4 charts, 9 sources) rafraîchi par collect-v2.py toutes les 15 min et déployé à H:10 :

```mermaid
flowchart LR
    subgraph "📊 Pipeline Dashboard"
        C[collect-v2.py<br/>*/15] -->|9 sources| J[leo-unified.json]
        J -->|H:10| D[Deploy Unified Dashboard]
        D -->|git push| GH[GitHub Pages]
        GH -->|CDN| WEB[🌍 leo-dashboard]
    end
```

| Dashboard | Contenu | URL | Collecte | Déploiement |
|-----------|---------|-----|----------|-------------|
| **LEO Dashboard** | Synthèse, Analyses, Infra, BAVI (+ 4 vaults) | [leo-dashboard](https://christophedanhier-hash.github.io/leo-dashboard/) | collect-v2.py */15 | H:10 via deploy-dashboard.py |
...
Le **leo-dashboard** consolide **9 sources unifiées** en une seule page :
- 📊 **Collecteur** — collect-v2.py, 9 sources, statut, durée
- 🔵 **Crons (16)** — statut, historique, erreurs (16/16 OK, 0 pause)
- 💰 **Budget DeepSeek** — ~$20.00 total estimé
- 🩺 **n8n** — 3 workflows, online/offline (via `localhost:5678`)
- 🏛️ **BAVI LEO** — sessions, messages, tokens
- 🖥️ **Machine** — statut en ligne (1 seul serveur)
...
> *Avec 16 crons qui tournent 24h/24 et un collecteur toutes les 15 min, le dashboard central synthétise tout : 20 KPI, 4 onglets (Synthèse, Analyses, Infra, BAVI), 4 charts Chart.js. Le tout dans **un seul fichier HTML statique** sur GitHub Pages.*
...
### Haute fréquence (15 min)

```yaml
- Unified Collector v2   → */15 → collect-v2.py (9 sources)
- Auto-Heal Agent         → */15 → détection + correction auto
- LEO Health Check bots   → */15 → vérifie les 4 gateways
```

### 30 min

```yaml
- Email Classifier (Ollama)  → */30 → classification Gmail
- sync-skills-to-copilot     → */30 → sync skills default → autres profils
```

### Horaires

```yaml
- Deploy Unified Dashboard   → H:10 → push leo-dashboard
- Déploiement auto tofdan.be → H:00 → mise à jour site
- drive-sync                 → H:00 → sync Drive → GitHub
```

### Multi-horaires (2h, 4h, 6h)

```yaml
- docs-update          → 0 */4 → mise à jour documentation
- doc-watch-auto       → 0 */6 → surveillance wikis
- Archive Watch        → 0 */6 → surveillance leo-tracker
```

### Quotidiens

```yaml
- LEO Full Backup complet   → 02:00 → backup profils → GDrive
- Veille IA quotidienne     → 07:00 → 15 sources RSS → analyse → email
- vault-daily-journal       → 23:00 → journal Obsidian (default)
- vault-default-daily-journal → 23:00 → journal Obsidian (leo-copilot)
```

### Bi-quotidiens

```yaml
- Budget Alert → 08:00, 20:00 → vérification solde DeepSeek
```
...
## Collecteur unifié collect-v2.py — 9 sources

Le collecteur `collect-v2.py` est le cœur du pipeline de données. Toutes les 15 minutes, il interroge **9 sources** et produit un fichier JSON unique `/opt/data/metrics/leo-unified.json`.

```mermaid
flowchart TB
    subgraph "📡 collect-v2.py — 9 sources unifiées"
        direction TB
        S1["① sessions<br/>Lit les state.db des 4 profils<br/>↓<br/>888 sessions, 20K messages<br/>$19.99 coût estimé"]
        S2["② budget<br/>API DeepSeek<br/>↓<br/>Solde $49.49<br/>Dépense 14j: $11.47"]
        S3["③ crons<br/>Scheduler Hermes<br/>↓<br/>16 OK, 0 error, 0 pause"]
        S4["④ infra<br/>Machine + services<br/>↓<br/>Gateway UP, Ollama UP<br/>n8n UP, 1 machine"]
        S5["⑤ n8n<br/>API localhost:5678<br/>↓<br/>3 workflows actifs"]
        S6["⑥ github<br/>API GitHub<br/>↓<br/>20 repos, activité 7j"]
        S7["⑦ bavi<br/>Index des bureaux<br/>↓<br/>5 bureaux actifs"]
        S8["⑧ services<br/>Spotify check<br/>↓<br/>1 service"]
        S9["⑨ vaults<br/>Scan 4 vaults Obsidian<br/>↓<br/>9 notes, 4 dailies"]
    end

    S1 --> JSON[leo-unified.json]
    S2 --> JSON
    S3 --> JSON
    S4 --> JSON
    S5 --> JSON
    ...
    JSON -->|H:10| DASH[🌍 LEO Dashboard<br/>4 onglets, 20 KPI]
```

**Performances** (dernière exécution au 04/07/2026 22:00) :
- Durée totale : 9.17 secondes
- Source la plus lente : GitHub API (8.2s)
- Source la plus rapide : services (0ms), vaults (1ms)
- 16 enregistrements budget, 17 sessions, 8 crons
- **0 alerte** — toutes les sources répondent OK
...
Budget mensuel LEO (estimé):
  ...
  Total:                           ~1-3 €
  Coût réel estimé total:         ~$19.99 (tous profils depuis début)
```
...
| Coût mensuel estimé  | ~1-3 € ($19.99 total)              |
| Veille IA            | ~5-10 cts/jour               |
| Crons                | 0 € (16/16 OK, majorité no_agent)      |
```
...
```yaml
Toutes les 15 minutes (minute 0/15/30/45):
  - 📊 Unified Collector v2  → collect-v2.py unifie 9 sources
  - 🩺 Auto-Heal Agent       → vérification santé système
  - 🩺 LEO Health Check bots → vérifie les 4 gateways

Toutes les 30 minutes (minute 0/30):
  - 📧 Email Classifier (Ollama) → classification Gmail
  - 🔄 sync-skills-to-copilot    → sync skills entre profils

Toutes les heures:
  - 🚀 Deploy Unified Dashboard   → H:10 → push leo-dashboard
  - 🔄 Déploiement auto tofdan.be → H:00
  - 🔄 drive-sync                 → H:00
```
...
H:00 → Déploiement auto tofdan.be
H:00 → drive-sync
H:10 → Deploy Unified Dashboard
H:15 → Unified Collector v2, Auto-Heal Agent
H:30 → Email Classifier, sync-skills-to-copilot
```
...
23:00 — Vault Daily Journal
  Action: Génération du journal quotidien Obsidian (default + leo-copilot)
  Coût: 0 € (no_agent)
```
...
Toutes les 15 minutes:
  - 🩺 Auto-Heal Agent          → crons, Ollama, n8n, Docker, disque, tokens
  - 📊 Unified Collector v2     → collecte 9 sources unifiées
  - 🩺 LEO Health Check bots    → vérifie les 4 gateways

Toutes les 30 minutes:
  - 📧 Email Classifier (Ollama) → nouveaux emails à classer
  - 🔄 sync-skills-to-copilot    → sync skills entre profils

Toutes les 2-6 heures:
  - 📖 doc-watch-auto            → détecte les changements wikis
  - 📦 Archive Watch             → surveillance leo-tracker

Tous les jours:
  - 💰 Budget Alert              → solde suffisant ?
  - 📓 Vault Daily Journal       → journal Obsidian
```
...
La documentation complète de l'écosystème LEO : 10 bureaux, 126 skills, 16 crons, 1 dashboard central. La preuve que Hermes peut gérer un assistant IA complet.
...
Profils : 4 (default, leo-copilot, bavi-leo, emile)
```
...
| Provider | Rôle | Coût |
|----------|------|------|
| DeepSeek 🤖 | Principal (Telegram, conversations, tâches complexes) | ~$20.00 total estimé |
| Ollama 🏠 | Local, gratuit (batch, traitement bulk, qwen2.5:7b) | Gratuit (CPU) |
| Gemini ⚡ | Fallback automatique (contexte 1M tokens) | Gratuit (quota API) |
...
### Crons (16 actifs, majorité no_agent)

| Vague | Horaires | Crons |
|:------|:---------|:------|
| **15 min** | `*/15` | 📊 Unified Collector v2, 🩺 Auto-Heal Agent, 🩺 LEO Health Check bots |
| **30 min** | `*/30` | 📧 Email Classifier (Ollama), 🔄 sync-skills-to-copilot |
| **Horaire** | H:00 | 🔄 Déploiement auto tofdan.be, 🔄 drive-sync |
| **Horaire** | H:10 | 🚀 Deploy Unified Dashboard |
| **4-6h** | `*/4`, `*/6` | 📝 docs-update, 📖 doc-watch-auto, 📦 Archive Watch |
| **Quotidien** | 02:00 | 💾 LEO Full Backup |
| **Quotidien** | 07:00 | 🔍 Veille IA (~0.05€/j) |
| **Bi-quotidien** | 08:00, 20:00 | 💰 Budget Alert |
| **Quotidien** | 23:00 | 📓 vault-daily-journal (default + leo-copilot) |

**Tous les crons sauf Veille IA sont en no_agent = 0€ LLM.**

### Workflows n8n (redondance)

Depuis juin 2026, certains crons critiques sont **doublés dans n8n** pour bénéficier du retry natif :

| Workflow n8n | Horaire | Rôle |
|-------------|---------|------|
| `Drive → Issue GitHub` | Webhook | Nouveau fichier Drive → Issue GitHub |
| `Gardien du Drive` | Continu | Surveillance modifications Drive |
| `Save Contacts` | Horaire | Sync Google Contacts |
...
|-----------|-------------|-----|------|
| 🦁 **LEO Dashboard** | HTML + Chart.js (4 onglets, 20 KPI) | */15 collect → H:10 deploy | [leo-dashboard](https://christophedanhier-hash.github.io/leo-dashboard/) |
...
- Validation des clés `leo-unified.json` au déploiement

## Chiffres clés (leo-unified.json, 04/07/2026)

| Métrique | Valeur |
|:---------|:-------|
| Sessions totales | **891** |
| Messages échangés | **20 226** |
| Appels API DeepSeek | **7 878** |
| Coût total estimé | **~$20.00** (tous profils) |
| Crons actifs | **16** (16/16 OK) |
| Workflows n8n | **3** |
| Skills installés | **126** |
| Vaults Obsidian | **4** |
| Bureaux actifs | **5** |
| Repos GitHub | **20** |
| Durée collecte | **~9.58s** (9 sources) |
...
### 4. Profils spécialisés

LEO a vécu la perte d'accès Telegram lors d'un basculement de profil. La solution retenue : **4 profils avec gateways indépendants**, mémoire partagée par symlinks. Chaque profil a son propre bot, provider et domaine.
