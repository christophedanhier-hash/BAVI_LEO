...
(function(){
  ...
  function loadBaviAnalytics() {
    fetch('/api/metrics?token='+token).then(function(r){return r.json()}).then(function(m){
      ...
      if(ctx1 && typeof Chart !== 'undefined') {
        new Chart(ctx1,{type:'bar',data:{labels:bu.map(function(d){return d.name}),datasets:[{data:bu.map(function(d){return d.analyses||0}),backgroundColor:bu.map(function(d){return colors[d.id]||'#94a3b8'}),borderRadius:4}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true,ticks:{stepSize:1}}}}});
      }
      ...
      if(ctx2 && sd.length && typeof Chart !== 'undefined') {
        new Chart(ctx2,{type:'line',data:{labels:sd.map(function(d){return d.date.slice(5)}),datasets:[{label:'Sessions',data:sd.map(function(d){return d.sessions}),borderColor:'#a78bfa',tension:0.3,fill:true,backgroundColor:'rgba(167,139,250,.1)',pointRadius:1}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}}}});
      }
    }).catch(function(){});
  }
  ...
  function buildGraphFilters() {
    ...
    fetch('/api/graph?token='+token).then(function(r){return r.json()}).then(function(data){
      ...
      var html = '<button onclick=\"loadBaviGraph(\\'\\')\" id=\"btn-all\" style=\"padding:4px 12px;background:var(--accent);color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:11px\">🌐 Tous</button>';
      bureaux.forEach(function(b){
        var c = colors[b]||'#94a3b8';
        html += '<button onclick=\"loadBaviGraph(\\''+b+'\\')\" class=\"graph-btn\" style=\"padding:4px 12px;background:var(--card);color:'+c+';border:1px solid '+c+';border-radius:4px;cursor:pointer;font-size:11px\">'+b.charAt(0).toUpperCase()+b.slice(1)+'</button>';
      });
      container.innerHTML = html;
      // Click handlers for styling
      container.querySelectorAll('button').forEach(function(btn){
        btn.addEventListener('click',function(){
          container.querySelectorAll('button').forEach(function(b){b.style.background='var(--card)'});
  ...
  // Also load immediately if visible
  setTimeout(function(){
  ...
  window.loadBaviGraph = loadBaviGraph;
})();
</script>
