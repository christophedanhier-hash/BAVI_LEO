...
<div class="card">
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:6px">
        {"".join(f'<div style="background:var(--card);border:1px solid var(--border);border-radius:8px;padding:8px;text-align:center"><div style="font-size:12px;font-weight:600">{"🦁" if k=="michel" else "🏠" if k=="default" else "🎓" if k=="emile" else "🚐"}</div><div style="font-size:14px;font-weight:700">{vv.get("dailies",0)}</div><div style="font-size:9px;color:var(--dim)">{"Michel" if k=="michel" else "Léo" if k=="default" else "Émile" if k=="emile" else "Bavi" if k=="bavi" else "Robert" if k=="robert" else k}</div></div>' for k,vv in va.get("vaults",{}).items())}
  </div>
